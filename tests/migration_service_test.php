...
migration_assert(count($migrations) === 22, 'Expected 22 migrations.');
migration_assert(array_keys($migrations) === range(1, 22), 'Migrations are not sequential.');
migration_assert($migrations[1]['filename'] === '001_database.display-groups-migration.sql', 'Unexpected first migration.');
migration_assert($migrations[22]['filename'] === '022_database.display-timezone-default-migration.sql', 'Unexpected last migration.');
