...
if (preg_match('#^/admin/displays/(\d+)/vnc$#', $uri, $m) && $method === 'GET') { $admin->displayVnc((int)$m[1]); exit; }
...
if (preg_match('#^/api/media/(\d+)/preview$#', $uri, $m) && $method === 'GET') { $frontend->mediaPreview((int)$m[1]); exit; }
