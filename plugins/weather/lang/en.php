<?php
return [
    'plugin.weather.name' => 'Weather',
    'plugin.weather.description' => 'Current weather display using Open-Meteo.',

    'plugin.weather.admin.title' => 'Weather plugin',
    'plugin.weather.admin.description' => 'Displays current weather for one selected location using Open-Meteo current weather data.',
    'plugin.weather.admin.free_notice' => 'Free Open-Meteo mode is enabled. The public endpoint is intended for non-commercial use. For commercial use, configure an API key and commercial endpoint in the plugin settings page.',
    'plugin.weather.admin.location_search' => 'Location search',
    'plugin.weather.admin.location_placeholder' => 'Search city or location',
    'plugin.weather.admin.search_help' => 'Type a city and save the slide. If the live search list is unavailable, the first matching location will be resolved automatically on save.',
    'plugin.weather.admin.selected_location' => 'Selected location',
    'plugin.weather.admin.timezone' => 'Timezone',
    'plugin.weather.admin.temperature_unit' => 'Temperature unit',
    'plugin.weather.admin.wind_speed_unit' => 'Wind speed unit',
    'plugin.weather.admin.precipitation_unit' => 'Precipitation unit',
    'plugin.weather.admin.show_datetime' => 'Show client date and time',
    'plugin.weather.admin.footer_note' => 'Weather data is fetched server-side and cached for one hour. The frontend clock uses the client browser time and timezone.',
    'plugin.weather.admin.search_no_results' => 'No locations found.',
    'plugin.weather.admin.search_failed' => 'Location lookup failed.',

    'plugin.weather.global.title' => 'Open-Meteo API settings',
    'plugin.weather.global.description' => 'Configure the Open-Meteo endpoints used by all weather slides. Leave the API key empty for the public non-commercial API.',
    'plugin.weather.global.weather_base_url' => 'Weather API endpoint',
    'plugin.weather.global.weather_base_url_placeholder' => 'https://api.open-meteo.com/v1/forecast',
    'plugin.weather.global.weather_base_url_help' => 'Default public endpoint: https://api.open-meteo.com/v1/forecast',
    'plugin.weather.global.geocoding_base_url' => 'Geocoding API endpoint',
    'plugin.weather.global.geocoding_base_url_placeholder' => 'https://geocoding-api.open-meteo.com/v1/search',
    'plugin.weather.global.geocoding_base_url_help' => 'Default public endpoint: https://geocoding-api.open-meteo.com/v1/search',
    'plugin.weather.global.api_key' => 'API key',
    'plugin.weather.global.api_key_placeholder' => 'Optional, e.g. open-meteo commercial key',
    'plugin.weather.global.api_key_help' => 'The key is stored in the database and sent only from server-side API requests.',
    'plugin.weather.global.cache_ttl' => 'Cache TTL in seconds',
    'plugin.weather.global.cache_ttl_help' => 'Number of seconds that fetched weather data may be reused. Default: 3600.',
    'plugin.weather.global.timeout' => 'HTTP timeout in seconds',
    'plugin.weather.global.timeout_help' => 'Minimum effective timeout is 3 seconds. Default: 12.',
    'plugin.weather.global.user_agent' => 'User-Agent header',
    'plugin.weather.global.user_agent_help' => 'Sent with server-side Open-Meteo requests. Default: Hugin Weather Plugin/1.0',

    'plugin.weather.unknown_location' => 'Unknown location',
    'plugin.weather.label.feels_like' => 'Feels like',
    'plugin.weather.label.wind' => 'Wind',
    'plugin.weather.label.precipitation' => 'Precipitation',

    'plugin.weather.condition.sunny' => 'Sunny',
    'plugin.weather.condition.clear_night' => 'Clear night',
    'plugin.weather.condition.cloudy' => 'Cloudy',
    'plugin.weather.condition.fog' => 'Fog',
    'plugin.weather.condition.rain' => 'Rain',
    'plugin.weather.condition.snow' => 'Snow',
    'plugin.weather.condition.storm' => 'Storm',
    'plugin.weather.condition.current_weather' => 'Current weather',
    'plugin.weather.condition.unknown' => 'Unknown',

    'plugin.weather.error.invalid_temperature_unit' => 'Weather plugin: invalid temperature unit.',
    'plugin.weather.error.invalid_wind_speed_unit' => 'Weather plugin: invalid wind speed unit.',
    'plugin.weather.error.invalid_precipitation_unit' => 'Weather plugin: invalid precipitation unit.',
    'plugin.weather.error.location_required' => 'Weather plugin: please search and select a location.',
    'plugin.weather.error.location_not_found' => 'Weather plugin: no matching location found.',
    'plugin.weather.error.invalid_latitude' => 'Weather plugin: invalid latitude.',
    'plugin.weather.error.invalid_longitude' => 'Weather plugin: invalid longitude.',
    'plugin.weather.error.current_weather_missing' => 'Weather plugin: weather API did not return current data.',
    'plugin.weather.error.api_request_failed' => 'Weather plugin: API request failed.',
    'plugin.weather.error.invalid_api_response' => 'Weather plugin: invalid API response.',
    'plugin.weather.error.invalid_weather_endpoint' => 'Weather plugin: invalid weather API endpoint.',
    'plugin.weather.error.invalid_geocoding_endpoint' => 'Weather plugin: invalid geocoding API endpoint.',
    'plugin.weather.error.invalid_cache_ttl' => 'Weather plugin: invalid cache TTL.',
    'plugin.weather.error.invalid_timeout' => 'Weather plugin: invalid HTTP timeout.',
];
