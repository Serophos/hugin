...
- Parser mapping is configured in the TL1 Menu setup panel and saved to `plugins/tl1-menu/config.php`.
- At runtime, the plugin loads global settings from the database and parser configuration from `plugins/tl1-menu/config.php`.
...
The TL1 Menu setup panel analyzes the configured XML feed and generates parser configuration into `plugins/tl1-menu/config.php`.
...
The generated `config.php` is intended to hold parser mappings only; operational defaults stay in the plugin global settings.
...
`plugins/tl1-menu/config.php` is required at runtime, generated by the setup panel, and contains parser configuration, including:
...
- Do not rely on the old monolithic `config.php` format; the plugin now uses a setup form for XML parser mapping.
