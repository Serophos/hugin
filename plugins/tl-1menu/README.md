...
The plugin now separates operational settings from generated parser setup:

- Hugin plugin global settings store the XML URL, cache TTL, debug date, slide defaults, environment toggles, background defaults, and environment icon paths. Existing non-mapping values from an old `config.php` are used as first-run defaults.
- `plugins/tl-1menu/config.php` is generated by the setup wizard and contains only parser mapping data: TL-1 metadata fields, target field mappings, price groups, locations, food types, categories, and token catalog entries.
- The setup wizard creates `config.php.YmdHis.bak` before replacing `config.php`. Old mapping arrays such as `pseudo_allergen_category_map`, `food_types`, `mensen`, `standort_namen`, and `categories` are not imported automatically. Re-run analysis and review the generated tables instead.
...
| --- | --- | --- |
| `environment_rating_icons` | array | Maps environmental metric keys such as `co2`, `water`, `animal_welfare`, and `rainforest` to plugin-relative icon paths or absolute URLs. Built-in paths from `assets/img/` resolve to their filled and outline variants; custom paths are used for both states. |
...
'categories' => [
    'vegan' => [
        'icon' => 'assets/img/categories/vegan.svg',
        'labels' => [
            'de' => 'Vegan',
            'en' => 'Vegan',
        ],
