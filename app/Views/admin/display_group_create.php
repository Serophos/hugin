...
<div class="card shadow-sm">
    <form method="post" action="<?= e(url('/admin/display-groups/create')) ?>" class="form-grid compact-grid">
        ...
        <label><?= e(__('common.sort_order')) ?>
            <input class="form-control" type="number" name="sort_order" value="<?= e((string)old('sort_order', '0', $formId)) ?>" min="0" placeholder="<?= e(__('display_groups.sort_order_placeholder')) ?>"<?= field_attrs('sort_order', $formId) ?>>
            <?= field_error_html('sort_order', $formId) ?>
        </label>
        <label class="full-width"><?= e(__('common.description')) ?>
            <textarea class="form-control" name="description" rows="3" placeholder="<?= e(__('display_groups.description_placeholder')) ?>"<?= field_attrs('description', $formId) ?>><?= e((string)old('description', '', $formId)) ?></textarea>
