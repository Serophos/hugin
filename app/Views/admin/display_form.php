...
<div class="display-form-layout">
    <div class="card">
        <form method="post" action="<?= e($display ? url('/admin/displays/' . $display['id'] . '/edit') : url('/admin/displays/create')) ?>" class="form-grid">
            <?= csrf_field() ?>
            <label><?= e(__('common.name')) ?><input type="text" id="display_name" name="name" value="<?= e($display['name'] ?? '') ?>" required></label>
            <label><?= e(__('display.human_url')) ?><input type="text" id="display_slug" name="slug" value="<?= e($display['slug'] ?? '') ?>" required><small class="field-note"><?= e(__('display.slug_autogenerated_hint')) ?></small></label>
