...
<form method="post" action="<?= e(url('/admin/settings')) ?>" class="form-grid settings-form">
    ...
    <fieldset class="card settings-card">
        ...
        <p class="text-body-secondary muted"><?= e(__('settings.branding_intro', [], 'Configure the default slide background, text color, and uploaded media-library fonts.')) ?></p>
        <div class="settings-card-grid">
            ...
            <label><?= e(__('settings.default_font_heading')) ?>
                <select class="form-select" name="settings[default_font_heading]"<?= field_attrs('default_font_heading', 'settings') ?>>
            ...
            <label><?= e(__('settings.default_font_text')) ?>
                <select class="form-select" name="settings[default_font_text]"<?= field_attrs('default_font_text', 'settings') ?>>
                    <option value=""><?= e(__('settings.system_default')) ?></option>
                    <?php foreach ($fonts as $font): ?>
                        <?php $fontToken = uploaded_font_token((int)$font['id']); ?>
        ...
        <?php if (!$fonts): ?>
            <div class="alert alert-warning warning"><?= e(__('settings.no_fonts_available')) ?></div>
        <?php else: ?>
            <p class="text-body-secondary small muted"><?= e(__('settings.font_directory_help')) ?></p>
        <?php endif; ?>
    </fieldset>
    <fieldset class="card settings-card">
        <legend><?= e(__('settings.system_heading', [], 'System')) ?></legend>
        <p class="text-body-secondary muted"><?= e(__('settings.system_intro', [], 'Configure application-wide behavior for the admin backend and displays.')) ?></p>
        <div class="settings-card-grid">
            <label><?= e(__('settings.locale', [], 'System language')) ?>
                <select class="form-select" name="settings[locale]"<?= field_attrs('locale', 'settings') ?>>
    ...
    <fieldset class="card settings-card">
        ...
        <p class="text-body-secondary muted"><?= e(__('settings.accessibility_intro', [], 'Configure compliant admin accessibility preferences and deployment-specific WAD contact details.')) ?></p>
        <div class="settings-card-grid">
            <label><?= e(__('settings.accessibility_visual_mode', [], 'Admin visual mode')) ?>
                <select class="form-select" name="settings[accessibility_visual_mode]"<?= field_attrs('accessibility_visual_mode', 'settings') ?>>
            ...
            <label><?= e(__('settings.accessibility_focus_style', [], 'Focus indicator')) ?>
                <select class="form-select" name="settings[accessibility_focus_style]"<?= field_attrs('accessibility_focus_style', 'settings') ?>>
            ...
            <label><?= e(__('settings.accessibility_motion', [], 'Motion')) ?>
                <select class="form-select" name="settings[accessibility_motion]"<?= field_attrs('accessibility_motion', 'settings') ?>>
            ...
            <label><?= e(__('settings.accessibility_contact_email', [], 'Accessibility contact email')) ?>
                <input class="form-control" type="email" name="settings[accessibility_contact_email]" value="<?= e((string)old('accessibility_contact_email', $settings['accessibility_contact_email'] ?? '', 'settings')) ?>" placeholder="accessibility@example.org"<?= field_attrs('accessibility_contact_email', 'settings') ?>>
            ...
            <label><?= e(__('settings.accessibility_feedback_url', [], 'Accessibility feedback URL')) ?>
                <input class="form-control" type="url" name="settings[accessibility_feedback_url]" value="<?= e((string)old('accessibility_feedback_url', $settings['accessibility_feedback_url'] ?? '', 'settings')) ?>" placeholder="https://example.org/accessibility-feedback"<?= field_attrs('accessibility_feedback_url', 'settings') ?>>
            ...
            <label><?= e(__('settings.accessibility_enforcement_url', [], 'Enforcement body URL')) ?>
                <input class="form-control" type="url" name="settings[accessibility_enforcement_url]" value="<?= e((string)old('accessibility_enforcement_url', $settings['accessibility_enforcement_url'] ?? '', 'settings')) ?>" placeholder="https://example.org/enforcement"<?= field_attrs('accessibility_enforcement_url', 'settings') ?>>
    ...
    <fieldset class="card settings-card">
        ...
        <p class="text-body-secondary muted"><?= e(__('settings.upload_intro', [], 'Set the maximum media upload size Hugin accepts. PHP and the web server must allow at least the same size.')) ?></p>
        <div class="settings-card-grid">
            <label><?= e(__('settings.upload_max_size_mb', [], 'Maximum upload size (MB)')) ?>
                <input class="form-control" type="number" name="settings[upload_max_size_mb]" min="1" step="1" value="<?= e((string)old('upload_max_size_mb', $settings['upload_max_size_mb'] ?? '50', 'settings')) ?>"<?= field_attrs('upload_max_size_mb', 'settings') ?>>
    ...
    <fieldset class="card settings-card">
        ...
        <p class="text-body-secondary muted"><?= e(__('settings.monitoring_intro', [], 'Configure external monitoring API access and heartbeat status thresholds.')) ?></p>
        <div class="settings-card-grid">
            ...
            <label><?= e(__('settings.monitoring_api_token', [], 'Bearer token')) ?>
                <input class="form-control" type="text" name="settings[monitoring_api_token]" value="<?= e((string)old('monitoring_api_token', $settings['monitoring_api_token'] ?? '', 'settings')) ?>" autocomplete="off"<?= field_attrs('monitoring_api_token', 'settings') ?>>
            ...
            <label><?= e(__('settings.monitoring_stale_threshold_seconds', [], 'Stale threshold (seconds)')) ?>
                <input class="form-control" type="number" name="settings[monitoring_stale_threshold_seconds]" min="30" step="1" value="<?= e((string)old('monitoring_stale_threshold_seconds', $settings['monitoring_stale_threshold_seconds'] ?? '1800', 'settings')) ?>"<?= field_attrs('monitoring_stale_threshold_seconds', 'settings') ?>>
