...
<div class="card">
    ...
    <table>
        ...
        <tbody class="sortable-list" data-sort-endpoint="<?= e(url('/admin/sort/playlists')) ?>" data-extra-name="display_id" data-extra-value="<?= e((string)$group['display']['id']) ?>">
        <?php foreach ($group['channels'] as $channel): ?>
            <tr draggable="true" data-id="<?= e((string)$channel['assignment_id']) ?>">
                ...
                <td class="actions">
                    ...
                    <form method="post" action="<?= e(url('/admin/playlists/' . $channel['channel_id'] . '/delete')) ?>" class="inline-form" onsubmit="return confirm(<?= json_encode(__('channel.delete_confirm')) ?>);">
                        <?= csrf_field() ?>
