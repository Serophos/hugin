...
<div class="card shadow-sm">
    <form method="post" action="<?= e($user ? url('/admin/users/' . $user['id'] . '/edit') : url('/admin/users/create')) ?>" class="form-grid">
        ...
        <label><?= e(__('auth.username')) ?>
            <input class="form-control" type="text" name="username" value="<?= e((string)old('username', $user['username'] ?? '', $formId)) ?>" placeholder="<?= e(__('users.username_placeholder')) ?>" required<?= field_attrs('username', $formId) ?>>
            <?= field_error_html('username', $formId) ?>
        </label>
        <label><?= e(__('users.display_name')) ?>
            <input class="form-control" type="text" name="display_name" value="<?= e((string)old('display_name', $user['display_name'] ?? '', $formId)) ?>" placeholder="<?= e(__('users.display_name_placeholder')) ?>"<?= field_attrs('display_name', $formId) ?>>
            <?= field_error_html('display_name', $formId) ?>
        </label>
        <label><?= e(__('users.role')) ?>
            <select class="form-select" name="role" required<?= field_attrs('role', $formId) ?>>
        ...
        <label><?= e(__('auth.password')) ?> <?= $user ? '<span class="text-body-secondary muted">' . e(__('auth.password_hint_keep')) . '</span>' : '' ?>
            <input class="form-control" type="password" name="password" placeholder="<?= e($user ? __('users.password_placeholder_edit') : __('users.password_placeholder_new')) ?>" <?= $user ? '' : 'required' ?><?= field_attrs('password', $formId) ?>>
        ...
        <div class="form-actions">
            ...
            <a class="btn btn-secondary button button--normal" href="<?= e(url('/admin/users')) ?>"><?= admin_icon('cancel') ?><span><?= e(__('common.cancel')) ?></span></a>
        </div>
    </form>
</div>
